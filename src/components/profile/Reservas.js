...
const Reservas = () => {
  ...
  const { user, logIn, updateUserInfo, logoutUser } = useContext(UserContext);
  ...
  return (
    <>
      ...
      <section className="profile__orders">
        ...
        <h3>Mis reservas</h3>
        <a href="https://procorlab.es/reservas" className="newBooking__link" style={{ margin: "3rem 4rem" }}>
          Nueva reserva
        </a>
        <div className="orders__column">
          {user.citas ? (
            user.citas.map((elm, idx) => (
              <article key={idx} className="order__byDay">
                ...
                <div className="order__card">
                  ...
                  <Link className="update__booking" id="SemiBold" to={`/modificar-reserva/${elm.id_cita}`}>
                    Modificar
                  </Link>
                </div>
              </article>
            ...
            <p className="aviso__certificados">No tiene citas</p>
          )}
        </div>
